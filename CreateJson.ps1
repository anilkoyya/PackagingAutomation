...
Param(
    ...
    [Parameter(Mandatory = $false)]
    [boolean]$HkcuRegistrySyncEnabled = $false,
    [Parameter(Mandatory = $false)]
    [string]$HkcuRegistrySyncTargetSid,
    [Parameter(Mandatory = $false)]
    [string[]]$HkcuRegistrySyncSubKeys = @('Software')
)
...
'@
    ...
    #Format HkcuRegistrySync subkeys for Json
    $HkcuRegistrySyncSubKeysjson = ($HkcuRegistrySyncSubKeys | ForEach-Object {
        '"' + $_.replace('\', '\\').replace('"', '\"') + '"'
    }) -join ", "
...
if ($Registrymodify){
    ...
    $RegistrymodifyNumber = 1
        foreach($Registry in $Registrymodify){
            ...
            $Registrymodifyjson += "`"Location`": `"" + $Registry.Location.replace('\', '\\' ).replace('"','\"') + "`",`n"
            if ($Registry.Action){
                $Registrymodifyjson += "`"Action`": `"" + $Registry.Action + "`",`n"
            }
...
if ($CaptureTimeoutSec -eq 1){
    ...
    $jsonfilebody = @"
    {
        ...
        "CaptureSettings": {
            ...
            "HkcuRegistrySync": {
                "Enabled": $HkcuRegistrySyncEnabled,
                "TargetSid": "$HkcuRegistrySyncTargetSid",
                "SubKeys": [$HkcuRegistrySyncSubKeysjson]
            }
        },
        "CaptureCommands": {
            "Enabled": true,
