...
$SUPORTED_JSON = 1.0, 1.1, 1.2, 1.3, 1.4, 1.5   #Add supported version in array
...
'
    ...
    if($hkcuSyncEnabled){
        # Appended last so it runs after the installer (and any custom PostInstallActions) but still inside
        # Installer.bat, which Studio executes and watches for the whole capture window - so the registry
        # writes this makes into the live HKEY_CURRENT_USER are captured just like the installer's own.
        $hkcuSyncSubKeysArgs = ($hkcuSyncSubKeys | ForEach-Object { "`"$_`"" }) -join ' '
        $hkcuAfterCmd = "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$hkcuSyncScript`" -Mode After -Sid `"$hkcuSyncSid`" -SnapshotFolder `"$hkcuSyncFolder`" -SubKeys $hkcuSyncSubKeysArgs`n"
        $batString = $batString + $hkcuAfterCmd
    }
...
function Initialize-RegData{
    ...
    foreach($entry in $json.ModifyAssets.ModifyKeys.PSObject.Properties.name){
        ...
        $entryValue = $json.ModifyAssets.ModifyKeys.PSObject.Properties[$entry].value
        $Location = $entryValue.PSObject.Properties["Location"].value
        $Keys = $entryValue.PSObject.Properties["Keys"].value

        $Action = "Add"
        if($entryValue.PSObject.Properties.name -contains "Action" -and $entryValue.Action){
        ...
        switch ($Action) {
            "DeleteKey" {
                #Deletes the entire key (and its subkeys/values) on the target machine
                $regString += "[-$Location]`n`n"
            }
            "DeleteValue" {
                #Deletes only the named values under the key, the key itself is left in place
                $regString += "[$Location]`n"
                foreach($valueName in $Keys){
                    $valueName = $valueName.Trim()
                    if($valueName -eq "@"){
                        #The key's unnamed/default value is not quoted in .reg syntax
                        $regString += "@=-`n"
                    }
                    else{
                        $regString += "`"$valueName`"=-`n"
                    }
                }
                $regString += "`n"
            }
        ...
        if($debug_mode){
            Write-Output "Creating registry file (Action=$Action):`n"
...
switch ($PsCmdlet.ParameterSetName)
{
    "jsonFile"  {
                    ...
                    # HKCU Registry Sync (JSON 1.5+): when packaging is driven remotely (e.g. WinRM) with no
                    # user interactively logged on, installers' per-user registry writes commonly land under
                    # HKEY_USERS\<SID> of a loaded profile rather than a live HKEY_CURRENT_USER, since there is
                    # no interactive session to map HKCU to - so Studio's capture never sees them. When enabled,
                    # Sync-HkcuFromUserSid.ps1 is wired in to snapshot that SID's hive before/after the install
                    # and replicate only what changed into this session's own live HKCU, which Studio does watch.
                    $hkcuSyncEnabled = $false
                    $hkcuSyncSid = $null
                    $hkcuSyncSubKeys = @("Software")
                    $hkcuSyncFolder = $null
                    $hkcuSyncScript = $null

                    if($json.CaptureSettings.HkcuRegistrySync -and $json.CaptureSettings.HkcuRegistrySync.Enabled)
                    {
                        $hkcuSyncEnabled = $true
                        $hkcuSyncSid = $json.CaptureSettings.HkcuRegistrySync.TargetSid
                        if($json.CaptureSettings.HkcuRegistrySync.SubKeys)
                        {
                            $hkcuSyncSubKeys = @($json.CaptureSettings.HkcuRegistrySync.SubKeys)
                        }

                        if([string]::IsNullOrWhiteSpace($hkcuSyncSid))
                        {
                            Throw "CaptureSettings.HkcuRegistrySync.Enabled is true but TargetSid is empty. Specify the SID of the user whose HKEY_USERS hive should be synced into HKEY_CURRENT_USER."
                        }

                        if(-NOT $json.CaptureCommands.Enabled)
                        {
                            # The "after" sync is appended to Installer.bat inside Initialize-InstallWrapper,
                            # which only runs when CaptureCommands.Enabled is true. Without it the "before"
                            # snapshot (and any hive this loads on demand) would never get cleaned up by an
                            # "after" pass, so fail fast instead of leaving that half-done.
                            Throw "CaptureSettings.HkcuRegistrySync requires CaptureCommands.Enabled to also be true, since the 'after' sync runs as part of the generated Installer.bat."
                        }

                        $hkcuSyncScript = Join-Path $PSScriptRoot "Sync-HkcuFromUserSid.ps1"
                        if(-NOT (Test-Path -Path $hkcuSyncScript))
                        {
                            Throw "CaptureSettings.HkcuRegistrySync is enabled but Sync-HkcuFromUserSid.ps1 was not found next to studio-nip.ps1 (expected at $hkcuSyncScript)."
                        }

                        $hkcuSyncFolder = Join-Path (Split-Path $config_file_path -Parent) "_HkcuSync"
                        Write-Output "HKCU registry sync enabled for SID $hkcuSyncSid (subkeys: $($hkcuSyncSubKeys -join ', '))"
                    }
                    ...
                    if($hkcuSyncEnabled)
                    {
                        # Runs before Studio starts capturing (Invoke-PreCaptureScript executes and waits for
                        # this synchronously), which is exactly when the "before" baseline needs to be taken.
                        $hkcuSyncSubKeysArgs = ($hkcuSyncSubKeys | ForEach-Object { "`"$_`"" }) -join ' '
                        $hkcuBeforeCmd = "powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$hkcuSyncScript`" -Mode Before -Sid `"$hkcuSyncSid`" -SnapshotFolder `"$hkcuSyncFolder`" -SubKeys $hkcuSyncSubKeysArgs"

                        if($json.PSObject.Properties.Name -contains 'PreCaptureCommands' -and $json.PreCaptureCommands)
                        {
                            $json.PreCaptureCommands = @($json.PreCaptureCommands) + $hkcuBeforeCmd
                        }
                        else
                        {
                            $json | Add-Member -NotePropertyName PreCaptureCommands -NotePropertyValue @($hkcuBeforeCmd) -Force
                        }
                    }
